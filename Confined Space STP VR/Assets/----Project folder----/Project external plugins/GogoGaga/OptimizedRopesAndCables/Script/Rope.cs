    ...
    public class Rope : MonoBehaviour
    {
        ...
        private void SetSplinePoint()
        {
            ...
            for (int i = 0; i < linePoints; i++)
            {
                ...
                Vector3 dir = p - previousPoint;
                float dist = dir.magnitude;

                if (Physics.Raycast(previousPoint, dir.normalized, out RaycastHit hit, dist))
                {
                    // Move rope point to collider surface
                    p = hit.point + hit.normal * 0.02f;
                }
        ...
        private Vector3 GetMidPoint()
        {
            ...
            Vector3 baseMid = Vector3.Lerp(startPointPosition, endPointPosition, midPointPosition);
            ...
            Vector3 dir = endPointPosition - startPointPosition;
            float dist = dir.magnitude;

            if (Physics.Raycast(startPointPosition, dir.normalized, out RaycastHit hit, dist))
            {
                // Edge point where rope touches collider
                Vector3 edgePoint = hit.point + hit.normal * 0.05f;

                // Blend the midpoint towards the collider edge
                float influence = 0.7f; // how strongly rope bends around obstacle
                baseMid = Vector3.Lerp(baseMid, edgePoint, influence);
            }
